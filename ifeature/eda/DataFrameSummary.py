...
class DataFrameSummary(object):
    ...
    def summary_plus(self):
        with ProcessPoolExecutor(min(7, self.df.shape[1])) as pool:
            _ = pool.map(self._get_column_summary, self.df.columns)
    ...
    def _get_top_correlations(self, column, threshold=0.65, top=3):
        ...
        return '｜'.join('{}:{:.2f}'.format(col, val) for col, val in correlations.items())
